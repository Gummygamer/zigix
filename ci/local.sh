...
run "smoke-parser-accepts-phase15" bash -c '
  tmp=$(mktemp)
  trap "rm -f $tmp" EXIT
...
run "smoke-parser-accepts-phase16-newlib-dirent" bash -c '
  tmp=$(mktemp)
  trap "rm -f $tmp" EXIT
  printf "[ZIGIX:BOOT:OK]\\n[ZIGIX:INIT:START]\\n[ZIGIX:TEST:PASS:newlib_c_runtime]\\n[ZIGIX:TEST:PASS:newlib_dirent]\\n[ZIGIX:INIT:OK]\\n" > "$tmp"
  tools/qemu/smoke_test.py "$tmp" --phase phase16-newlib-dirent
'
...
#    kernel build + qemu smoke. This is the real Phase 15 acceptance gate.
if [[ -n "${ZIGIX_BUN_ZIG:-}" ]]; then
  ...
  if command -v qemu-system-x86_64 >/dev/null 2>&1; then
    run "qemu-smoke-phase15" tools/toolchain/zig-bun build qemu-smoke
    run "qemu-smoke-scripted-phase12-interactive" tools/toolchain/zig-bun build qemu-smoke-scripted
  else
    skip "qemu-smoke-phase15" "qemu-system-x86_64 not installed"
  ...
  skip "qemu-smoke-phase15" "ZIGIX_BUN_ZIG is not set"
