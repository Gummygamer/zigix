...
#    qemu smoke. This is the real Phase 4 acceptance gate.
if [[ -n "${ZIGIX_BUN_ZIG:-}" ]]; then
  ...
  if command -v qemu-system-x86_64 >/dev/null 2>&1; then
    run "qemu-smoke-phase4" tools/toolchain/zig-bun build qemu-smoke
  else
    skip "qemu-smoke-phase4" "qemu-system-x86_64 not installed"
  fi
else
  ...
  skip "qemu-smoke-phase4" "ZIGIX_BUN_ZIG is not set"
fi
