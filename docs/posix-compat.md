...
| Toybox source build | partial | pinned upstream `echo.c`, `cat.c`, and the `nproc` implementation in `taskset.c` boot as separate ELF processes through newlib using a narrow Toybox header/runtime overlay; `cat` exercises file I/O and `nproc --all` counts a deterministic sysfs directory via `readdir`; affinity syscalls remain unsupported | `toybox`, `toybox_cat`, `toybox_nproc`, `newlib_dirent`, `cpu_sse`, `port_source_locks` |
