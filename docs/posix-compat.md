...
| newlib syscall hooks | partial | `_read`, `_write`, `_open`, `_close`, `_dup2`, `_lseek`, `_fstat`, `_stat`, `_isatty`, `_getpid`, `_kill`, `_sbrk`, `_exit`; `_sbrk`/`_kill` are deliberate stubs | `libc_shim_newlib`, host `libc_shim`, `syscall_dup2` |
