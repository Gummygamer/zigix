...
| Toybox source build | partial | pinned upstream `echo.c`, `cat.c`, `taskset.c` (`nproc`), and `id.c` boot as separate ELF processes through a narrow newlib overlay; `cat` exercises file I/O, `nproc --all` consumes `readdir`, and numeric `id -u` observes the single-user credential ABI; affinity syscalls remain unsupported | `toybox`, `toybox_cat`, `toybox_nproc`, `toybox_id`, `newlib_dirent`, `cpu_sse`, `syscall_credentials`, `port_source_locks` |
