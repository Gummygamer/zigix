...
| `getcwd` | partial | normalized per-process absolute cwd; newlib wrapper supports caller buffers and allocates a 256-byte buffer for `getcwd(NULL, 0)` | `syscall_getcwd`, `toybox_pwd` |
| `getuid` / `geteuid` / `getgid` / `getegid` | partial | explicit single-user-root policy; all return `0`; no per-process credentials or permission enforcement yet | `syscall_credentials`, `toybox_id` |
...
| Toybox source build | partial | pinned upstream `echo.c`, `cat.c`, `taskset.c` (`nproc`), `id.c`, and `pwd.c` boot separately through a narrow newlib overlay; file, directory, process-identity, and shell-oriented paths are proven, but the full multicall runtime is not ported | `toybox`, `toybox_cat`, `toybox_nproc`, `toybox_id`, `toybox_pwd`, `newlib_dirent`, `cpu_sse`, `syscall_credentials`, `syscall_getcwd`, `port_source_locks` |
