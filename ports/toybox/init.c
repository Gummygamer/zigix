...
void _start(void) {
  ...
  saved_stdout = syscall1(32, 1);
  capture = syscall3(2, (long)"/pwd-output", 02 | 0100 | 01000, 0);
  if (saved_stdout < 3 || capture < 3 || syscall2(33, capture, 1) != 1) {
    say("[ZIGIX:TEST:FAIL:toybox_pwd:redirect]\n");
    quit(1);
  }
  syscall1(3, capture);

  pid = syscall3(4000, (long)"/toybox-pwd", 0, 0);
  if (pid <= 0) {
    syscall2(33, saved_stdout, 1);
    say("[ZIGIX:TEST:FAIL:toybox_pwd:spawn]\n");
    quit(1);
  }
  status = -1;
  waited = syscall4(61, pid, (long)&status, 0, 0);
  ...
  syscall1(3, saved_stdout);
  if (waited != pid || status != 0) {
    say("[ZIGIX:TEST:FAIL:toybox_pwd:wait]\n");
    quit(1);
  }

  capture = syscall3(2, (long)"/pwd-output", 0, 0);
  output[0] = output[1] = output[2] = output[3] = 0;
  amount = capture < 3 ? -1 : syscall3(0, capture, (long)output, sizeof(output));
  if (capture >= 3) syscall1(3, capture);
  if (amount != 2 || output[0] != '/' || output[1] != '\n') {
    say("[ZIGIX:TEST:FAIL:toybox_pwd:output]\n");
    quit(1);
  }
  say("[ZIGIX:TEST:PASS:toybox_pwd]\n");
